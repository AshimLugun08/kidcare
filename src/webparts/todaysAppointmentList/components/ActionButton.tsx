...
import { baseAPI } from "../../kidDetailList/components/EnvironmentVariables";
...
export default function ActionButton(props: any) {
  ...
  const SendMail = async () => {
    try {
      const url = `${baseAPI()}/SendMail?ToEmail=${props.Kid_Email}&Name=${
        props.parent_Name
      }&Subject=Appointment reminder&Message=${
        props.appointment_Type ? Email_walking : Email_Tele
      }`;
